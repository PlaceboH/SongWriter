...
export class UsersPageComponent implements OnInit {
    ...
    ngOnInit(): void {
        this.userService.getCurrentUser().subscribe((myUser) => {
            this.myUser = myUser;
            this.followingUsers$ = this.userService.getFollowingUsers(
                this.myUser.id
            );
            this.userService
                .getAllUsers()
                .pipe(
                    map((data) =>
                        data.filter(
                            (user: { id: number }) => user.id != this.myUser.id
                        )
                    )
                )
                .subscribe((usersData) => {
    ...
    private getImagesToUsers(users: User[]): void {
        users.forEach((user) => {
            this.imageService.getImageToUser(user.id as number).subscribe({
                next: (data) => {
